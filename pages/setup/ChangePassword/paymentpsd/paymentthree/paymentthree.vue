<template>
	<view class="content" @touchstart="touchstart" @touchmove="touchmove">
		<view class="content">
			<view class="con-login">设置支付新密码</view>
			<view class="inputs" style="display: flex;justify-content: space-between;align-items:center;">
				<input :type="psd" class="phone" style="font-size: 1.2rem;" v-model="UserPsd.text" placeholder="6位数字"/>
...
<script>
	...
	export default {
		...
		onLoad(options){
				this.UserRegister.old_user_trade_password=options.oldpsd
		},
			...
			completelogin(){
				this.$api.ChekcToken(this.$store.state.UserLogin.SessionFlag).then((res)=>{
					...
					if(this.PSDflag){
						...
						this.UserRegister.user_trade_password=this.UserPsd.text
						var mdoldpsd=md5(this.UserRegister.old_user_trade_password)
						var jsonesc=Utils.encrypt(JSON.stringify(this.UserRegister),mdoldpsd)
						var PsdJson={
						...
						console.log(jsonesc)
						console.log(PsdJson)
						this.$api.ChangeUserPwd(PsdJson).then((res)=>{
							console.log(res)
							if(res.data.result==0){
